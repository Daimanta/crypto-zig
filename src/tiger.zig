...
pub const Tiger = struct {
    ...
    fn process_block(self: *Self) void {
        var cast_buffer: []u64 = @alignCast(mem.bytesAsSlice(u64, self.buffer[0..]));
    ...
    fn round(a: *u64, b: *u64, c: *u64, x: u64, mul: u8) void {
        c.* ^= x;
        a.* -%= sboxes[0][@as(u8, @truncate(c.*))] ^
            sboxes[1][@as(u8, @truncate((c.*) >> (2 * 8)))] ^
            sboxes[2][@as(u8, @truncate((c.*) >> (4 * 8)))] ^
            sboxes[3][@as(u8, @truncate((c.*) >> (6 * 8)))];
        b.* +%= sboxes[3][@as(u8, @truncate((c.*) >> (1 * 8)))] ^
            sboxes[2][@as(u8, @truncate((c.*) >> (3 * 8)))] ^
            sboxes[1][@as(u8, @truncate((c.*) >> (5 * 8)))] ^
            sboxes[0][@as(u8, @truncate((c.*) >> (7 * 8)))];
        b.* *%= mul;
    }
