...
const hash_bytes: u8 = 24;
const hash_string_size: u8 = 2 * hash_bytes;

const sboxes: [4][256]u64 = .{
    .{
...
pub const Tiger = struct {
    ...
    pub fn init() Self {
        var result = Self{ .hash = undefined, .buffer = undefined, .datasize_in_buffer = undefined, .length = undefined, .tiger_2 = undefined };
        result.reset();
        return result;
    }
    ...
    pub fn make_final(self: *Self, digest: *[24]u8) []u8 {
        ...
        index += 1;
        if (index > 56) {
            mem.set(u8, self.buffer[56..], 0)            ;
            self.process_block();
            index = 0;
        }
        
        mem.set(u8, self.buffer[index..56], 0);
    ...
    pub fn reset(self: *Self) void {
        ...
        self.datasize_in_buffer = 0;
        self.tiger_2 = false;
        self.hash = .{0x0123456789ABCDEF, 0xFEDCBA9876543210, 0xF096A5B4C3B2E187};
    }

    fn process_block(self: *Self) void {
        
        var cast_buffer: []u64 = @alignCast(8, mem.bytesAsSlice(u64, self.buffer[0..]));
        ...
        key_schedule(&x);
        pass(&hash_copy[2], &hash_copy[0], &hash_copy[1], x, 7);
        key_schedule(&x);
        pass(&hash_copy[1], &hash_copy[2], &hash_copy[0], x, 9);
        
        self.hash[0] ^= hash_copy[0];
        self.hash[1] = hash_copy[1] -% self.hash[1];
        self.hash[2] +%= hash_copy[2];
    }
    
    fn pass(a: *u64, b: *u64, c: *u64, x: [8]u64, mul: u8) void {
        round(a,b,c,x[0],mul);
        round(b,c,a,x[1],mul);
        round(c,a,b,x[2],mul);
        round(a,b,c,x[3],mul);
    ...
    fn round(a: *u64, b: *u64, c: *u64, x: u64, mul: u8) void {
        c.* ^= x;
        a.* -%= sboxes[0][@truncate(u8, c.*)] ^
            sboxes[1][@truncate(u8,(c.*) >> (2 * 8))] ^
            sboxes[2][@truncate(u8, (c.*) >> (4 * 8))] ^
            sboxes[3][@truncate(u8, (c.*) >> (6 * 8))] ;
        b.* +%= sboxes[3][@truncate(u8, (c.*) >> (1 * 8))] ^
            sboxes[2][@truncate(u8, (c.*) >> (3 * 8))] ^
            sboxes[1][@truncate(u8, (c.*) >> (5 * 8))] ^
            sboxes[0][@truncate(u8, (c.*) >> (7 * 8))];
        b.* *%= mul;
    }
    
    fn key_schedule(x: *[8]u64) void {
...
fn output_hash(comptime length: u8, hash:[length]u8, string: *[2 * length]u8) void {
...
fn output_192(hash: [hash_bytes]u8, string: *[hash_string_size]u8) void {
    output_hash(24, hash, string);
}
...
test "empty string" {
    ...
    var processor = Tiger.init();
    _ = processor.make_final(&input);
    
    var string: [hash_string_size]u8 = undefined;
    output_192(input, &string);
    
    const expected: []const u8 = "3293ac630c13f0245f92bbb1766e16167a4e58492dde73f3";
...
test "test strings" {
    var maps: [2]TextPair = .{
        TextPair{.first = "The quick brown fox jumps over the lazy dog", .second = "6d12a41e72e644f017b6f0e2f7b44c6285f06dd5d2c5b075"}, 
        TextPair{.first = "The quick brown fox jumps over the lazy cog", .second = "a8f04b0f7201a0d728101c9d26525b31764a3493fcd8458f"}
    };
