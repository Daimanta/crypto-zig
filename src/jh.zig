...
pub const JH = struct {
    ...
    pub fn make_final(self: *Self, digest: *[128]u8) []u8 {
        ...
        if (self.datasize_in_buffer == 0) {
            ...
            var i: usize = 63;
            while (i >= 56) : (i -= 1) {
                self.buffer[i] = @truncate(bit_length >> @as(u6, @intCast((63 - i) * 8)));
            }

            self.F8();
        } else {
            @memset(self.buffer[self.datasize_in_buffer..], 0);
            ...
            var j: usize = 63;
            while (j >= 56) : (j -= 1) {
                self.buffer[j] = @truncate(bit_length >> @as(u6, @intCast((63 - j) * 8)));
            }
            self.F8();
        }
    ...
    pub fn reset(self: *Self) void {
        ...
        self.H[0] = @truncate(bitlen >> 8);
        self.H[1] = @truncate(bitlen);
    ...
    fn E8_initialgroup(self: *Self) void {
        ...
        for (temp, 0..) |_, i| {
            var j: usize = 0;
            while (j < 4) : (j += 1) {
                t[j] = @truncate(self.H[(i + j * 256) >> 3] >> @intCast((7 - (i & 7))));
            }
            temp[i] = (@as(u4, t[0]) << 3) | (@as(u4, t[1]) << 2) | (@as(u4, t[2]) << 1) | (@as(u4, t[3]) << 0);
        }

        for (temp[0..128], 0..) |_, i| {
            const j = i * 2;
            self.A[j] = temp[i];
    ...
    fn E8_finaldegroup(self: *Self) void {
        ...
        for (temp, 0..) |byte, i| {
            var j: usize = 0;
            while (j < 4) : (j += 1) {
                t[j] = @truncate(byte >> @as(u2, @intCast(3 - j)));
            }
            j = 0;
            while (j < 4) : (j += 1) {
    ...
    fn R8(self: *Self) void {
        ...
        while (i < 256) : (i += 1) {
            roundconstant_expanded[i] = @truncate(self.roundconstant[i >> 2] >> @intCast((3 - (i & 3))));
        }
