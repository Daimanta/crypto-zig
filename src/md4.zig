...
pub const Md4 = struct {
    ...
    pub fn update(self: *Self, buf: []const u8) void {
        ...
        if (buf.len + self.partial_bytes < BLOCK_SIZE) {
            @memcpy(self.partial[self.partial_bytes..buf.len], buf[0..]);
            ...
            if (remaining_in_buffer > 0) {
                ...
                const to_copy = buf.len - (taken_from_buffer + taken);
                @memcpy(self.partial[0..to_copy], buf[taken_from_buffer + taken ..]);
    ...
    pub fn make_final(self: *Self, digest: *[HASH_UNITS]u32) void {
        var ints: *[MESSAGE_UNITS]u32 = @alignCast(mem.bytesAsSlice(u32, self.partial[0..])[0..16]);
    ...
    fn process_partials(self: *Self) void {
        self.process_block(mem.bytesAsSlice(u32, self.partial[0..])[0..MESSAGE_UNITS].*);
    }
...
fn digest_to_hex_string(digest: *[HASH_UNITS]u32, string: *[2 * HASH_SIZE]u8) void {
    ...
    while (i < digest.len) : (i += 1) {
        ...
        _ = std.fmt.bufPrint(string[start..end], "{X:0>8}", .{my_num}) catch {};
    }
}
